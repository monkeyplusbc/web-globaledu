...
<template>
  <header class="relative wrapper !bg-secundary">
            ...
            <ul class="navbar-nav">
              <template v-for="(item, i) in menu.items" :key="i">
                <li v-if="item.header" class="nav-item dropdown dropdown">
                  <atoms-button :data="item.link" class="nav-link !text-[.85rem] !tracking-[normal]"
                    :data-bs-toggle="{'dropdown': Array.isArray(item.submenu) && item.submenu.length > 0}"
                  ...
                  <ul v-if="Array.isArray(item.submenu) && item.submenu.length > 0" class="dropdown-menu">
                    <li v-for="(subitem, j) in item.submenu" :key="j" class="dropdown dropdown-submenu dropend">
                      <atoms-button :data="subitem.link" class="dropdown-item hover:!text-primary"
                        :data-bs-toggle="{'dropdown': Array.isArray(subitem?.submenu) && subitem?.submenu.length > 0}"
            ...
            <div class="offcanvas-footer xl:hidden lg:hidden">
              <div>
                ...
                <nav class="nav social social-white mt-4">
                  <atoms-button v-for="(item, i) in networks.items" :key="i"
        ...
        <div class="navbar-other w-full !flex !ml-auto">
          <ul class="navbar-nav !flex-row !items-center !ml-auto">
            ...
            <li v-if="menu.help.active" class="nav-item">
              <a class="nav-link hover:!text-[#fab758]" data-bs-toggle="offcanvas"
                data-bs-target="#offcanvas-info"><mk-icon>{{
    ...
    <div v-if="menu.help.active" id="offcanvas-info"
